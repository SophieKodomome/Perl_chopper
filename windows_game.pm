...
sub draw_helicopter {
    ...
    $Helicopter->{x2} = $Helicopter->getX1() + $helicopter_image->width();
    $Helicopter->{y2} = $Helicopter->getY1() + $helicopter_image->height();
...
sub move_helicopter {
    ...
    if ($Helicopter->move($move_right, $move_left, $move_up, \@obstacles)) {
        ...
        $self->{text} = $self->{canvas}->createImage(800, 350, -image => $win);

        my $text = "Click r to restart...";
        $self->{text} = $self->{canvas}->createText(800, 500, -text => $text, -font => "{Broken Console Bold} 14", -fill => "#FFD5FC");
